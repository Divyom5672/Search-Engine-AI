...
from langchain.callbacks import StreamlitCallbackHandler ##to display the thoughts and actions of an agent in streamlit.
import os
...
if prompt := st.chat_input(placeholder="Ask me anything about research papers or Wikipedia topics..."):
    st.session_state.messages.append({"role": "user", "content": prompt})
    st.chat_message("user").write(prompt)

    llm=ChatGroq(groq_api_key = api_key, model_name = "openai/gpt-oss-120b", streaming=True)
    tools = [arxiv, wiki, Search]
    llm = llm.bind_tools(tools) # without this line, groq tells model to not use tools
